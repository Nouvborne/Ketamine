...
struct SettingsView: View {
    ...
    private func detectLocalAuthHash() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            do {
                let hash = try BadQuery.findLocalAuthenticationHash().trimmingCharacters(in: .whitespacesAndNewlines)
                DispatchQueue.main.async {
                    localAuthHash = hash
    ...
    private func detectInCallHash() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            do {
                let hash = try BadQuery.findInCallHash().trimmingCharacters(in: .whitespacesAndNewlines)
                DispatchQueue.main.async {
                    inCallHash = hash
